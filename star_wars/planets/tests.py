...
from planets.services import PlanetService, UserPlanetService


class PlanetListTestCase(APITestCase):
    fixtures = ['users.yaml', 'movies.json', 'planets.json']

    def setUp(self):
        self.user_service = UserService()
        self.planet_service = PlanetService()
        self.user_planet_service = UserPlanetService()
        self.user = self.user_service.get_all_user().first()
        self.url = reverse(
    ...
    def test_search_without_custom_name(self):
        planet = self.planet_service.get_all_planets().first()
    ...
    def test_search_with_custom_name(self):
        planet = self.planet_service.get_all_planets().first()
        user_planet = self.user_planet_service.create_or_update_user_planet(
            planet=planet,
            user=self.user,
            is_favorite=True,
    ...
    def test_search_custom_planet_by_orignal_name(self):
        planet = self.planet_service.get_all_planets().first()
        self.user_planet_service.create_or_update_user_planet(
            planet=planet,
            user=self.user,
            is_favorite=False,
            custom_name="some_random_name"
...
class UserPlanetCreateTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.user_planet_service = UserPlanetService()
        self.user = self.user_service.get_all_user().first()
        self.palnet = self.planet_service.get_all_planets().first()
        self.url = reverse(
            'planets:create_user_planet',
            kwargs={"planet_id": self.palnet.id}
        )
        self.invalid_planet_url = reverse(
            'planets:create_user_planet',
            kwargs={"planet_id": 9999999999}
        )
